"""
Banking domain prompt templates for GPT-OSS-20B model
"""
...
class BankingPrompts:
    ...
    # --- MODIFIED: Enhanced the persona and role description ---
...
3. Formulate a detailed, step-by-step `resolution_suggestion` that a junior support agent could follow to resolve the issue.
...
Response must be in this exact JSON format:
{
  ...
  "original_input": "Complete original customer text",
  "severity": "Low|Medium|High|Critical",
  "impacted_department": "Account Management|Credit Cards|Loans and Mortgages|Investment Services|Customer Service|Technical Support|Fraud Prevention|Compliance|Wire Transfers|Mobile Banking",
  "impacted_service": "Specific service affected",
  "customer_name": "Customer name if provided",
  "customer_id": "Customer ID if provided",
  "resolution_suggestion": "A detailed, step-by-step action plan for an internal support agent. It should be professional, comprehensive, and provide clear instructions to resolve the issue or escalate it appropriately. Include specific internal actions where applicable.",
  "preferred_communication": "based on user input its either Email|Phone|SMS|In-Person|Mobile App|Online Chat|Mail|Video Call, if not specified then it's Not Specified",
  "ticket_id": "Original ticket ID from the source system"
}
...
Respond with only "BANKING" if related to financial services, or "NOT_BANKING" if unrelated."""
    ...
    @classmethod
    def get_ticket_generation_prompt(cls, customer_text: str, customer_name: Optional[str] = None, 
    ...
    @classmethod
    def get_validation_prompt(cls, customer_text: str) -> str:
        """
        Generate prompt for banking domain validation
        """
        return cls.VALIDATION_PROMPT.format(customer_text=customer_text)

    # --- MODIFIED: Significantly upgraded the resolution_suggestion in examples ---
    EXAMPLES = [
        {
  ...
  "severity": "High",
  "impacted_department": "Technical Support",
  "impacted_service": "Mobile Banking Application",
  "customer_name": "Not provided",
  "customer_id": "Not provided",
  "resolution_suggestion": "1. Verify customer identity using the standard security Q&A protocol. 2. Advise customer to perform basic troubleshooting: a) Ensure the app is updated to the latest version. b) Restart their mobile device. c) Clear the app's cache. 3. If the issue persists, create a Tier 2 technical support ticket using the 'Mobile Banking' template in the CRM. 4. Document the customer's device model, operating system version, and the exact time of the crash in the ticket. 5. Inform the customer that the issue has been escalated and provide the new ticket number for their reference.",
